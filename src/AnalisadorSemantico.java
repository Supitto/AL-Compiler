...
public class AnalisadorSemantico extends LinguagemAlgoritimicaBaseVisitor<Map<String, String>>{
    ...
    static void adicionarErro(String msgErro, int numLinha) {
        erro += "Linha " + numLinha + ": " + msgErro + "\n";
    }
    ...
    {
        FunctionArgs fa = new FunctionArgs(ctx.ID(),visitTipo_estendido(ctx.tipo_estendido()).get("Tipo"), new List<HashMap.Entry<String,String>>());
    ...
    public Map<String, String> visitMais_ident(LinguagemAlgoritimicaParser.Mais_identContext ctx) {
        ...
        if (ctx.identificador() != null) {
            ...
            saida.put("Nome" + (size + 1), nomeIdent);
    ...
    public Map<String, String> visitParcela_unario(LinguagemAlgoritimicaParser.Parcela_unarioContext ctx) {
        ...
        /* else if (ctx.ID() != null)
        {
            if(!main.tabelas.funcaoExiste(ctx.ID()))
            {
